...
## OCR→整形ワークフロー（`ocr-format`）

`FORMAT_LLM` に整形用のモデルを設定すると、モデル一覧に仮想モデル `ocr-format` が追加されます（未設定の場合は表示されません）。これを選んで PDF を添付すると、次の手順を順に実行し、全ページの内容を 1 つの整形済みの表にまとめます。途中で使うモデルを切り替えます（OCR は `glm-ocr`、整形は `FORMAT_LLM`）。

1. **OCR**：全ページ（またはチャットで指定したページ）を `glm-ocr` で表として読み取ります。
2. **結合**：ページごとの表を 1 つにまとめます。空の行や、ページごとに繰り返される見出し行は除きます。
3. **日付の分割（ルール）**：`12/5K12/8T` のように 1 つのセルに入った日付や記号を分け、最初の日付を日付の列に、残りの日付や記号を備考に入れます。
4. **整形（LLM）**：残りの列をテンプレートの列に割り当てます。氏名を姓と名に分けたり、明らかな OCR の誤りを直したりします。出力は JSON スキーマで固定します。
5. **書式の統一と検証（ルール）**：日付・郵便番号・電話番号の書き方をそろえ、形式が正しいかを確認します。
6. **出力**：Markdown の表と Excel ファイル（「整形済み」「確認リスト」「OCR 原本」の 3 シート）を返します。

出力する列はテンプレートで決まります。既定値の `日付,姓,名,郵便番号,住所,電話番号,備考` は住所録形式の台帳を想定した例です。扱う書類に合わせて `FORMAT_TEMPLATE` で設定してください。また、チャットで `列: 日付, 氏名, 住所` のように書くと、その回だけ変更できます。

LLM の出力はそのまま使わず、元の行と照らし合わせて検証します。

| 印 | 意味 |
| --- | --- |
| ✎ 補正 | 元の行にない値になっています（LLM による修正）。元の行を確認リストに残します。 |
| ⚠ 要確認 | 日付・郵便番号・電話番号の形式が正しくありません。 |
| ⚠ 欠落の可能性 | 元の行にあった英数字が、整形後の行に見当たりません。 |
| ⚠ 未整形 | LLM の出力がなかったため、元の列をそのまま使いました。 |

整形のあとに同じ会話で質問を続けると、`FORMAT_LLM` が回答します。OCR の結果はキャッシュするので、読み取りをやり直すことはありません。

これは決まった手順を実行する「ワークフロー」で、LLM が自分で次の行動を決める「エージェント」ではありません。データを勝手に変えないことを優先し、LLM に任せる範囲を列の割り当てと軽い補正に限っています。
...
│   │
│   └── formatter.py
│       └── OCR→整形ワークフロー（結合・LLM 整形・検証・Excel）
│
├── librechat/
...
- OCR→整形ワークフローを使う場合は、構造化出力（JSON スキーマ）に対応した整形用のモデルを Ollama に用意し、`FORMAT_LLM` に設定すること

### 推奨するコンテキストサイズ

Ollama の既定のコンテキストサイズ（4096）では、`glm-ocr` が 1 ページの画像だけでほぼ上限に達し、読み取り結果が途中で切れることがあります。モデルごとに `num_ctx` を設定してください。

```sh
printf 'FROM glm-ocr:latest\nPARAMETER num_ctx 16384\n' > Modelfile
ollama create glm-ocr:latest -f Modelfile
```

- `glm-ocr`：16384 以上（ページ画像で約 4K トークン、出力上限 `OCR_MAX_TOKENS` の既定値が 8192 のため）
- 整形用のモデル：`FORMAT_BATCH_ROWS` 行分の入力と出力が収まるサイズ（目安は 8192 以上）
- `ocr-format` では OCR 用と整形用の 2 つのモデルが同時にメモリに載るため、合計のメモリ使用量を確認してください。`ollama ps` で実際の使用量を確認できます。
- `ollama pull` でモデルを取得し直すと、この設定は元に戻ります。
...
| `FORMAT_MODEL_NAME` | `ocr-format` | OCR→整形ワークフローとしてモデル一覧に表示する名前 |
| `FORMAT_LLM` | （空） | 整形に使うモデル。JSON スキーマによる構造化出力に対応したモデルを指定します。空の場合、`ocr-format` は無効になります。 |
| `FORMAT_TEMPLATE` | `日付,姓,名,郵便番号,住所,電話番号,備考` | 整形後の表の列（カンマ区切り）。既定値は住所録形式の台帳向けの例です。 |
| `FORMAT_BATCH_ROWS` | `15` | LLM に一度に渡す行数 |
...
- OCR→整形の結果には、印の付いていない誤りも残ることがあります。重要なデータは、Excel の「OCR 原本」シートと見比べて確認してください。
