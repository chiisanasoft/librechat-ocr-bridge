...
モデル一覧に仮想モデル `ocr-format` が追加されます。これを選んで PDF を添付すると、次の手順を順に実行し、全ページの内容を 1 つの整形済みの表にまとめます。途中で使うモデルを切り替えます（OCR は `glm-ocr`、整形は `FORMAT_LLM`）。
...
出力する列はテンプレートで決まります。既定値は `FORMAT_TEMPLATE` で設定し、チャットで `列: 日付, 氏名, 住所` のように書くと、その回だけ変更できます。
...
| `FORMAT_LLM` | `nemotron-nano-9b-ja:latest` | 整形に使うモデル。JSON スキーマによる構造化出力に対応したモデルを指定します。 |
| `FORMAT_TEMPLATE` | `日付,姓,名,郵便番号,住所,電話番号,備考` | 整形後の表の列（カンマ区切り） |
